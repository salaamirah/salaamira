(() => {
  ...
  const elShip      = $('#ship');
  const elGrand     = $('#grand');
  const elShippingType = $('#shippingType');
  ...
  function validateDiscount(code, subtotal){
    ...
    if(meta.minSubtotal && subtotal<meta.minSubtotal) return {ok:false,reason:`Requires minimum ${fmt(meta.minSubtotal)}`};
  ...
  function applyDiscountCodeFromInput(){
    ...
    if(!res.ok){ setAppliedDiscount({code,meta:null}); ckDiscountMsg.textContent=res.reason; renderCartLines(); return; }
    setAppliedDiscount({code:res.code,meta:res.meta}); ckDiscountMsg.textContent=`${res.code} applied — ${res.meta.label||''}`; renderCartLines();
  }
  ...
  function renderProducts(items){
    ...
    items.forEach(p=>{
      ...
      `;
      $('.add',card).addEventListener('click',e=>{e.preventDefault(); e.stopPropagation(); addToCart({id:p.id,title:p.title,price:p.price,qty:1,image:p.image});});
  ...
  function renderCartLines(){
    const cart=getCart();
    const shippingType=elShippingType?.value||'domestic';
  ...
  function openCheckout(){
    ...
    const cart=getCart();
    const shippingType=elShippingType?.value||'domestic';
    const t=totals(cart,APPLIED_DISCOUNT,shippingType);
    ckItemsCount.textContent=String(cart.reduce((s,i)=>s+Number(i.qty),0));
    ckSub.textContent=fmt(t.subtotal); ckTax.textContent=fmt(t.tax); ckTotal.textContent=fmt(t.grand);
  ...
  elShippingType?.addEventListener('change',()=>renderCartLines());
  ...
  function showList(){ if(elMain) elMain.hidden=false; if(elProductPage) elProductPage.hidden=true; document.title='MiniShop'; }
    ...
    if(elProductPage){
      ...
      elProductPage.innerHTML = `
        <div class="product-detail">
          <button id="backToList" class="btn">← Back to catalog</button>
          <h2>${p.title}</h2>
          ${p.image?`<img src="${p.image}" alt="${p.title}" class="product-image">`:''}
          <p>${p.description}</p>
          <div class="price-row"><strong>${fmt(p.price)}</strong></div>
          <button class="btn primary" id="addProductDetail">Add to Cart</button>
        </div>
      `;
      $('#backToList')?.addEventListener('click',()=>{ location.hash = '/'; });
      $('#addProductDetail')?.addEventListener('click',()=>addToCart({id:p.id,title:p.title,price:p.price,image:p.image,qty:1}));
    }
